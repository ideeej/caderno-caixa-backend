...
describe('CashRegister Entity', () => {
  ...
  describe('Initialization', () => {
    it('should create an instance with default properties and correct initial balance', () => {
      ...
      const initialCash = new Money('50')
  ...
  describe('Core Functionality', () => {
    it('should correctly register a new deposit and withdraws in the balance', () => {
      const cashRegister = makeCashRegister({
        balance: { cash: new Money('50') },
      })

      cashRegister.deposit({ type: PaymentType.CASH, amount: new Money('50') })

      expect(cashRegister.balance.cash).toEqual(new Money('100'))

      cashRegister.withdraw({ type: PaymentType.CASH, amount: new Money('50') })
      expect(cashRegister.balance.cash).toEqual(new Money('50'))
    })

    it('should correctly close the cash register with the declared amount', () => {
      const declaredAmount = new Money('100')
      const cashRegister = makeCashRegister({})
  ...
  describe('Business Rules and Edge Cases', () => {
    ...
    it('should not allow deposits when the cash register is closed', () => {
      const declaredAmount = new Money('100')
      const amountToDeposit = new Money('10')
      const amountToWithdraw = new Money('10')
      const cashRegister = makeCashRegister({})
